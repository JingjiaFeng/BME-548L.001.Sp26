...
from data_sidd_iso import SIDDIsoDataset, get_split
...
def quick_eval(model, val_loader, device, max_batches=32):
    ...
    psnrs = []
    with torch.no_grad():
        for i, (noisy, gt, iso_embed, _) in enumerate(val_loader):
            if i >= max_batches:
                break
            _, _, H, W = noisy.shape
            ...
            iso_embed = iso_embed.to(device)
            
            out = torch.clamp(model(noisy, iso_embed), 0, 1)
...
def main():
    ...
    train_ds = SIDDIsoDataset(train_folders, mode='train', patch_size=256)
    ...
    for it in range(start_iter, args.total_iter):
        noisy, gt, iso_embed, _ = next(train_iter)
        noisy = noisy.to(device, non_blocking=True)
        gt = gt.to(device, non_blocking=True)
        iso_embed = iso_embed.to(device, non_blocking=True)
        
        pred = model(noisy, iso_embed)
