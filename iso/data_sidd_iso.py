...
class SIDDIsoDataset(Dataset):
    ...
    def __init__(self, folder_list, mode='train', patch_size=256):
        ...
        for folder in folder_list:
            ...
            assert len(noisy_files) == len(gt_files), f"{folder}: mismatched noisy/gt count"
            for nf, gf in zip(noisy_files, gt_files):
                self.samples.append((nf, gf, meta['iso']))
        
        print(f"[SIDDIsoDataset/{mode}] {len(folder_list)} scenes, {len(self.samples)} image pairs")
    ...
    def __getitem__(self, idx):
        noisy_path, gt_path, iso = self.samples[idx]
        ...
        iso_tensor = torch.tensor([[float(iso)]])  # shape [1,1]
        iso_embed = embed_param(iso_tensor, ISO_MIN, ISO_MAX).squeeze(0)  # → [9]
        
        return noisy_t, gt_t, iso_embed, iso

if __name__ == '__main__':
    train_folders, val_folders = get_split()
    print(f"\nSplit: {len(train_folders)} train / {len(val_folders)} val scenes")
    
    from collections import Counter
    train_isos = Counter(parse_scene_folder(f)['iso'] for f in train_folders)
    val_isos = Counter(parse_scene_folder(f)['iso'] for f in val_folders)
    print(f"\nTrain ISO distribution: {sorted(train_isos.items())}")
    print(f"Val ISO distribution:   {sorted(val_isos.items())}")
    
    train_ds = SIDDIsoDataset(train_folders, mode='train')
    noisy, gt, iso_embed, iso = train_ds[0]
    print(f"\n[train sample 0]")
    print(f"  noisy: {noisy.shape} {noisy.dtype} range=[{noisy.min():.3f}, {noisy.max():.3f}]")
    print(f"  gt:    {gt.shape} {gt.dtype}")
    print(f"  iso_embed: {iso_embed.shape} = {iso_embed.numpy().round(3)}")
    print(f"  iso scalar: {iso}")
    
    val_ds = SIDDIsoDataset(val_folders, mode='val')
    noisy, gt, iso_embed, iso = val_ds[0]
    print(f"\n[val sample 0]")
    print(f"  noisy: {noisy.shape} (full res, multiple of 8)")
    print(f"  iso: {iso}")
