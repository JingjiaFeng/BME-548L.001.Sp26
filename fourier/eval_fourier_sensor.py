"""
Fourier Domain Analysis for SENSOR conditioning — averaged over ALL val patches.
Same checkpoint, NoCond (zero embed) vs Sensor Injection (real sensor embed).
"""
...
from data_sidd_sensor import SIDDSensorDataset, get_split, SENSOR_MIN, SENSOR_MAX
...
def main():
    ...
    print("Loading Sensor Injection model...")
    ...
    version_names = ['Noisy', 'NoCond', 'Sensor Injection', 'Ground Truth']
    ...
    with torch.no_grad():
        for i in range(len(val_ds)):
            ...
            for np_p, gt_p in zip(noisy_patches, gt_patches):
                ...
                sensor_emb = sensor_embed.unsqueeze(0).to(device)
                out_sensor = torch.clamp(model(inp, sensor_emb), 0, 1).squeeze(0).cpu().numpy()

                versions = {
                    'Noisy': np_p.numpy(),
                    'NoCond': out_nc,
                    'Sensor Injection': out_sensor,
            ...
            print(f"  [{i+1:3d}/{len(val_ds)}] iso={iso:5d} phone={phone} "
                  f"patches={len(noisy_patches)} total={n_patches_total}")

    print(f"\nTotal patches: {n_patches_total}")
    ...
    colors = {'Noisy': '#E63946', 'NoCond': '#808080',
              'Sensor Injection': '#06A77D', 'Ground Truth': '#2E86AB'}

    # ==================== FIGURE 1: Radial Power ====================
    print("\nPlotting radial power spectrum...")
    ...
    for ax_idx, (lo, hi, bucket_name) in enumerate(ISO_BUCKETS):
        ...
        for vname in version_names:
            ...
            lw = 2.5 if vname in ('Sensor Injection', 'Ground Truth') else 1.5
    ...
    fig.suptitle('Averaged Radial Power Spectrum: NoCond vs Sensor Injection (same checkpoint)\n'
                 f'(Averaged over {n_patches_total} patches)',
                 fontsize=13, fontweight='bold', y=1.03)
    plt.tight_layout()
    plt.savefig('fourier_results/radial_power_sensor.png', dpi=170, bbox_inches='tight')
    print("✅ Saved fourier_results/radial_power_sensor.png")
    plt.close()

    # ==================== FIGURE 2: HF Preservation ====================
    print("Plotting HF preservation...")
    fig, ax = plt.subplots(figsize=(10, 6.5))

    nocond_devs, ours_devs, bucket_labels, n_counts = [], [], [], []

    for lo, hi, bucket_name in ISO_BUCKETS:
        key_gt = (bucket_name, 'Ground Truth')
        key_nc = (bucket_name, 'NoCond')
        key_s = (bucket_name, 'Sensor Injection')
        if key_gt not in avg_bands:
            continue
        gt_hf = avg_bands[key_gt]['High']
        nc_hf = avg_bands[key_nc]['High']
        s_hf = avg_bands[key_s]['High']
        nocond_devs.append(abs(nc_hf - gt_hf) / gt_hf * 100)
        ours_devs.append(abs(s_hf - gt_hf) / gt_hf * 100)
    ...
    ax.bar(x - w/2, nocond_devs, w, label='NoCond (zero embedding)',
           color='#808080', alpha=0.88, edgecolor='black', linewidth=0.5)
    ax.bar(x + w/2, ours_devs, w, label='Sensor Injection (real sensor)',
           color='#06A77D', alpha=0.88, edgecolor='black', linewidth=0.5)
    ...
    ax.set_title('High-Frequency Preservation: NoCond vs Sensor Injection\n'
                 f'(Averaged over {n_patches_total} patches — pure sensor conditioning contribution)',
                 fontsize=12, pad=10)
    ax.legend(fontsize=11, loc='upper left')
    ax.grid(True, axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig('fourier_results/hf_preservation_sensor.png', dpi=170, bbox_inches='tight')
    print("✅ Saved fourier_results/hf_preservation_sensor.png")
    plt.close()

    # Summary
    print(f"\n{'='*70}")
    print(f"Sensor HF Deviation from GT (%)")
    print(f"{'='*70}")
    print(f"{'Bucket':<20} {'N':<8} {'NoCond':<12} {'Sensor Inj':<12} {'Δ':<12}")
    print('-' * 70)
    for i, (_, _, name) in enumerate(ISO_BUCKETS):
        if i < len(nocond_devs):
            diff = nocond_devs[i] - ours_devs[i]
            print(f"{name:<20} {n_counts[i]:<8} {nocond_devs[i]:<12.2f} "
                  f"{ours_devs[i]:<12.2f} {'+' if diff > 0 else ''}{diff:<12.2f}")
